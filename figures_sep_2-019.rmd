...
```{r}
# prep fpkm file
# jun_fpkm <- read.table('human_fpkm.tsv', header = TRUE, stringsAsFactors = FALSE)
# jun_fpkm <- dplyr::rename(fpkm,
#                                BM_P5_11       = C23_N728_S505,
#                                BM_P10_11      = B04_N704_S503,
#                                BM_P12_11      = B06_N706_S503,
#                                CS19_P5_11     = B08_N710_S503,
...
#                                w13_FL_P12_19  = F18_N722_S508)
# 
# rownames(jun_fpkm) <- jun_fpkm$ensgene
# 
# nov_20_fpkm <- read.table('legion/20_nov_18/temp2', header = TRUE, stringsAsFactors = FALSE)
# rownames(nov_20_fpkm) <- nov_20_fpkm$ensgene
# 
# nov_20_fpkm <- nov_20_fpkm[which(rownames(nov_20_fpkm) %in% rownames(jun_fpkm)),]
# jun_fpkm<-jun_fpkm[which(rownames(jun_fpkm) %in% rownames(nov_20_fpkm)),]
# big_fpkm  <- cbind(jun_fpkm,nov_20_fpkm)
# big_fpkm$ensgene <- NULL
# big_fpkm$ensgene <- NULL
# big_fpkm <- big_fpkm[,-34]
save(big_fpkm, file = 'data/big_fpkm.RData')
```
